...
-- Desktop template
   ...
   commands = 
        [ Run DiskU [("/", "<fn=1>\xf7c9</fn><usedp>%")] 
          ...
          Run DynNetwork [ "--template" , "<fc=#83A598><fn=1>\xf0aa</fn></fc> <tx>KB/s <fc=#B8BB26><fn=1>\xf0ab</fn></fc> <rx>KB/s", "--Low", "1000", "--High", "5000", "--low", "#F2E5BC", "--normal", "#F2E5BC", "--high", "#F2E5BC" ] 600,
          Run MultiCpu [ "--template", "<fn=1>\xf85a</fn> <total>%" , "--Low", "50", "--High", "85", "--low", "#F2E5BC" , "--normal","#F2E5BC","--high", "#F2E5BC" ] 600,
          Run MultiCoreTemp [ "-t", "<max>°C ", "-L", "60", "-H", "80", "-l", "#F2E5BC", "-n", "#F2E5BC", "-h", "#F2E5BC", "--", "--mintemp", "20", "--maxtemp", "100" ] 600,
          Run Memory [ "--template", "<fn=1>\xf0c7</fn> <usedratio>%", "--Low", "20", "--High", "90", "--low", "#F2E5BC", "--normal", "#F2E5BC", "--high", "#F2E5BC" ] 600,
          ...
          Run Com "/home/anthony/utils/gpu-usage" ["<fn=1>\xf878</fn>"] "gpu" 600,
